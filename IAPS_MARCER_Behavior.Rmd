...
```{r rating check, warning=FALSE, message=FALSE,fig.height=3.5, fig.width=7.75, fig.align='center'}
...
```{r normative, warning=FALSE, message=FALSE,fig.height=3.5, fig.width=7.75, fig.align='center'}
...
## Function for graphing2
```{r function2, warning=FALSE, message=FALSE}
make.plot <- function(do.group) { # do.group <- "MDD status"; do.group <- "do.all";
  colors <- c("green", "blue") 
  if (do.group == "age") {
    group <- c("MA", "OA");}
  if (do.group == "MDD status") {
    ...
    for (gid in 1:length(group)) {
    plot(x=0, y=0, xlim=c(0.5, length(trial.type) +0.5), ylim=c(0-0.05,7+0.05), xaxt = 'n', ylab="", xlab="", main="", cex.axis=0.85, col='white');
  if (do.group == "age" | do.group == "MDD status") {
      if (do.group == "age") {
        ttl <- paste0(group[gid] , " Group - ", length(unique(data$sub.id[which(data$age.group == group[gid])])), " participants");
      }
